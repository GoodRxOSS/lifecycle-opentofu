...
|------|-------------|------|---------|:--------:|
| <a name="input_dns_domain"></a> [dns\_domain](#input\_dns\_domain) | The DNS domain name under which the DNS record will be created.<br/>Example: "example.com" or "app.example.com" | `string` | n/a | yes |
| <a name="input_dns_proxied"></a> [dns\_proxied](#input\_dns\_proxied) | Whether the record gets the Cloudflare 'orange cloud' (true) or is just a regular DNS record (false). <br/>If true, traffic is routed through the Cloudflare network, enabling features like WAF, CDN, and DDoS protection.<br/>Note: Proxied records must use a TTL ("var.dns\_ttl") of 1 (automatic). | `bool` | `false` | no |
| <a name="input_dns_record_name"></a> [dns\_record\_name](#input\_dns\_record\_name) | Subdomain or hostname. Use "*" for wildcard. <br/>If null, the record is created for the root dns\_domain. | `string` | `null` | no |
